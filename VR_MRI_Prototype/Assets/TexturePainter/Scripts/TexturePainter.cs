...
public class TexturePainter : MonoBehaviour
{
    ...
    float brushSize = .03f; //The size of our brush
    Color brushColor; //The selected color
    int brushCounter = 0, MAX_BRUSH_COUNT = 3000; //To avoid having millions of brushes
    ...
    public void ButtonPressed()
    {
        pressing = true;
        brushSize = .5f;
        brushCursor.transform.localScale = new Vector3(brushSize, brushSize, brushSize);
    }
    ...
    bool HitTestUVPosition(ref Vector3 uvWorldPosition)
    {
        ...
        Ray brushRay = new Ray(brush.transform.position, brush.forward);
        if (Physics.Raycast(brushRay, out hit, pressing ? 1.5f : 0.08f))
        {
